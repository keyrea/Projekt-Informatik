...
public class AtributeChange : MonoBehaviour 
{
    ...
    public void changeAtribute(string answer_type){// тип ответа да/нет
        ...
        string result = "";
        int new_current_day = int.Parse(get_current_day.text.ToString()) + 1;
        ...
        // если текущее значение опустилось до 0 или ниже, перейти в главное меню и обнулить атрибуты
        if (new_current_mental <= 0 || new_current_health <= 0 ||
            new_current_progress <= 0 || new_current_money <= 0){
                myGameObject.GetComponent<ChangeScene>().NextSceneAndToDefault(0);
        } else { //иначе перейти в меню выбора вопроса
